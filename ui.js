...
function populateSelectors() {
  ...
  // Срочность (абонентское)
  const urgencyDiv = getElement(CONFIG.DOM_IDS.URGENCY);
  if (urgencyDiv) {
    Object.entries(URGENCY).forEach(([key, urg], index) => {
      const label = document.createElement('label');
      label.className = 'radio';
      label.innerHTML = `
        <input type="radio" name="urgency" value="${escapeHtml(key)}" ${index === 0 ? 'checked' : ''}>
        ${escapeHtml(urg.name)}
      `;
      urgencyDiv.appendChild(label);
    });
  }
...
function renderSubscriptionDetails(result) {
  ...
  // НДС
  const vatText = result.vatMode === 'no' ? 'не облагается' : 
                  result.vatMode === 'sep' ? `+${formatPrice(result.vatAmount)}` : 'включён';
  lines.push(`НДС: ${vatText.padStart(32)}`);
...
function handleCalculate() {
  ...
  if (type === 'subscription') {
    ...
    // Предупреждение об экстренной срочности для абонентского
    if (result.urgency.coefficient > 1.2) {
...
function restoreFromParams() {
  ...
  if (type === 'subscription') {
    ...
    if (urgency) {
      const urgRadio = document.querySelector(`input[name=urgency][value="${escapeHtml(urgency)}"]`);
